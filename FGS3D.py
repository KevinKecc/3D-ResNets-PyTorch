...
class FGS3D(nn.Module):
    ...
    def __init__(self, num_classes=400, num_frames=64, num_keyframe=8, dropout_keep_prob=0.5, phase='train'):
        ...
        if phase == 'train':
            self.logits = nn.Linear((384+384+128+128)*32, 400)
            self.resnet_feature.fc = nn.Linear(num_ftrs, 400)

            state_dict = torch.load('/data2/Meva/pretrainedmodel/save_33.pth')
            state_dict = state_dict['state_dict']
            new_state_dict = OrderedDict()
            for k, v in state_dict.items():
                #if 'resnet_feature' not in k:
                name = k[7:]  # remove `module.`
                new_state_dict[name] = v
            # state_dict = torch.load('/data2/Meva/result/trainA/finetuneimgLr0.001//save_34.pth')
            # state_dict = state_dict['state_dict']
            # for k, v in state_dict.items():
            #     if 'resnet_feature' in k:
            #         name = k[7:]  # remove `module.`
            #         new_state_dict[name] = v
        ...
        self.resnet_feature.fc = nn.Linear(num_ftrs, self.num_classes)
        torch.nn.init.normal_(self.resnet_feature.fc.weight, mean=0.0, std=0.01)
        torch.nn.init.constant_(self.resnet_feature.fc.bias, 0.0)
        ...
        if phase == 'test':
            state_dict = torch.load('/data2/Meva/result/trainA/vidlr0.001/save_71.pth')
            state_dict = state_dict['state_dict']
            new_state_dict = OrderedDict()
            for k, v in state_dict.items():
                name = k[7:]  # remove `module.`
