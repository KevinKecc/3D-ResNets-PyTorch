...
class FGS3D(nn.Module):
    ...
    def __init__(self, num_classes=400, num_frames=64, num_keyframe=8, dropout_keep_prob=0.5, phase='train'):
        ...
        if phase =='train':
            state_dict = torch.load('/data/Kinetics400/result/finetunelr0.1/A10epochsLr0.1/save_22.pth')
            ...
            state_dict = torch.load('/data/Kinetics400/result/finetunelr0.1/A10epochsLr0.1/A35epochs/save_47.pth')
            state_dict = state_dict['state_dict']
            new_state_dict = OrderedDict()
            for k, v in state_dict.items():
                name = k[7:]  # remove `module.`
