...
from train_img_vid import train_epoch
from validation import val_epoch
import test
from FGS3D_img_vid import FGS3D
...
if __name__ == '__main__':
    ...
    if opt.arch == 'FGS3D-0':
        ...
        if 'trainA' in opt.annotation_path:
            split = 'trainA'
        elif 'trainB' in opt.annotation_path:
            split = 'trainB'
        elif 'trainC' in opt.annotation_path:
            split = 'trainC'
        else:
            print('no split {}'.format(split))
        model = FGS3D(num_classes=num_class, dropout_keep_prob=0.0, split=split)
    ...
    for i in range(opt.begin_epoch, opt.n_epochs + 1):
        if not opt.no_train:
            train_acc = train_epoch(i, train_loader, model, criterion, optimizer, opt,
